...
namespace GeeksCoreLibrary.Components.Configurator.Models
{
    internal class ConfiguratorDefaultSettingsModel 
    {
        ...
        /// <summary>
        /// The name of the current configurator, when the <see cref="ConfiguratorCmsSettingsModel.ComponentMode"/> is <see cref="Configurator.ComponentModes.Default" />.
        /// </summary>
        [DefaultValue("{configurator}")]
        public string ConfiguratorName { get; }

        /// <summary>
        /// The query that retrieves the basic data for the configurator, such as the templates for all steps, when the <see cref="ConfiguratorCmsSettingsModel.ComponentMode"/> is <see cref="Configurator.ComponentModes.Default" />.
        /// </summary>
        [DefaultValue(@"SELECT
    #SUMMARY
    ...
    IF(IFNULL(mainstepvalues.urlregex, '') <> '', mainstepvalues.urlregex, IF(IFNULL(stepvalues.urlregex, '') <> '', stepvalues.urlregex, IFNULL(substepvalues.urlregex, ''))) AS urlregex
...
WHERE
    configurator.moduleid = 320
    AND configurator.`level` = 1
    AND configurator.`name` = ?name

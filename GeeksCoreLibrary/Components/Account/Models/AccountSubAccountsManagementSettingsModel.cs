...
{
    [DefaultValue("""
                  <!-- There must always be a element with ID GclAccountContainer{contentId}, unless you also overwrite the TemplateJavascript. -->
    ...
    [DefaultValue("""
                  SELECT 'Dit subaccount bestaat al' AS error
                  FROM wiser_item AS account
    ...
    [DefaultValue("""
                  INSERT INTO wiser_item (entity_type, moduleid, title, added_by) VALUES (?subAccountEntityType, 600, ?login, 'website');
    ...
    [DefaultValue("""
                  UPDATE wiser_item AS subAccount
    ...
    [DefaultValue("""
                  INSERT INTO wiser_itemdetail (item_id, `key`, value) VALUES (?subAccountId, ?name, ?value)

...
internal class AccountCreateOrUpdateAccountSettingsModel
{
    ...
    [DefaultValue("""
                  SELECT 'Dit account bestaat al' AS error
    ...
    [DefaultValue("""
                  SELECT password.value AS password
                  FROM wiser_item AS account
    ...
    [DefaultValue("""
                  INSERT INTO wiser_item (entity_type, moduleid, title, added_by) VALUES (?entityType, 600, ?login, 'website');
    ...
    [DefaultValue("""
                  INSERT INTO wiser_itemdetail (item_id, `key`, value) VALUES (?userId, ?name, ?value)

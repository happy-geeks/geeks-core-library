...
public class DatabaseHealthService : IHealthCheck
{
    ...
    public DatabaseHealthService(IDatabaseConnection databaseConnection, IHttpContextAccessor httpContextAccessor)
    {
        ...
        CancellationToken cancellationToken = new CancellationToken())
    {
        ...
        var query =
            "SELECT ID,USER, HOST,TIME AS connection_time_in_sec, CASE WHEN TIME > 60 THEN 'Unhealthy' ELSE 'Healthy' END AS connection_status FROM information_schema.PROCESSLIST WHERE TIME >5000";
        ...
        foreach (DataRow row in datatable.Rows)
        {
            ...
            switch (state)
            {
                ...
                case "active":
                {
                    ...
                    {
                        errors.Add(
                            $"Service {row.Field<string>("configuration")} with time ID {row.Field<int>("time_id")} should have started more than 5 minutes ago");
                    }
                ...
                case "running":
                    ...
                    if (runningLongerThanMinutes.HasValue)
                    {
                        ...
                        {
                            errors.Add(
                                $"Service {row.Field<string>("configuration")} with time ID {row.Field<int>("time_id")} has been running for longer than {runningLongerThanMinutes.Value} minutes");
                        }
                    }
                    break;
                case "stopped":
                    errors.Add(
                        $"Service {row.Field<string>("configuration")} with time ID {row.Field<int>("time_id")} is stopped");
                    break;
                case "crashed":
                    errors.Add(
                        $"Service {row.Field<string>("configuration")} with time ID {row.Field<int>("time_id")} has crashed");
                    break;
                case "failed":
                    errors.Add(
                        $"Service {row.Field<string>("configuration")} with time ID {row.Field<int>("time_id")} has failed");
                    break;
                case "warning":
                {
                    ...
                    {
                        errors.Add(
                            $"Service {row.Field<string>("configuration")} with time ID {row.Field<int>("time_id")} is in warning state");
                    }
                ...
                default:
                    errors.Add(
                        $"Service {row.Field<string>("configuration")} with time ID {row.Field<int>("time_id")} is in unknown state: {state}");
                    break;
            }
        } 
        if (errors.Any())
